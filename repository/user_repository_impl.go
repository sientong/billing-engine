...
func (repository *UserRepositoryImpl) Create(ctx context.Context, tx *sql.Tx, user domain.User) domain.User {
	...
	if existingUser.ID != "" {
		helper.PanicIfError(fmt.Errorf("user with identity number %s already exists", user.IdentityNumber))
	}
	...
	if err != nil {
		helper.PanicIfError(fmt.Errorf("error inserting user: %w", err))
	}
...
func (repository *UserRepositoryImpl) Update(ctx context.Context, tx *sql.Tx, user domain.User) domain.User {
	existingUser, err := repository.FindByIdentityNumber(ctx, tx, user.IdentityNumber)
	if err != nil && err != sql.ErrNoRows {
		helper.PanicIfError(fmt.Errorf("error finding user by identity number: %w", err))
	}

	if existingUser.ID == "" {
		helper.PanicIfError(fmt.Errorf("user with identity number %s does not exist", user.IdentityNumber))
	}
	...
	if err != nil {
		helper.PanicIfError(fmt.Errorf("error updating user: %w", err))
	}

	return user
}

func (repository *UserRepositoryImpl) Delete(ctx context.Context, tx *sql.Tx, user domain.User) domain.User {
	...
	if err != nil {
		helper.PanicIfError(fmt.Errorf("error deleting user: %w", err))
	}
